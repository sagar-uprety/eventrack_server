...
const register = async (req, res) => {
  try {
    ...
    if (!event) return res.json({ message: "Cannot Find Event.", state: false });
    else {
      var message = "";
      if (!event.registeredUsers.includes(currentUser._id)) {
        ...
        event.registeredUsers.splice(event.registeredUsers.indexOf(currentUser._id), 1);
        currentUser.registeredEvents.splice(currentUser.registeredEvents.indexOf(event._id), 1);
    ...
    const events = await Event.find(
      {
        ...
        "dateTime.dates.0": date ? { $gte: date[0], $lte: date[1] } : { $exists: true },
      },
...
const getParticpants = async (req, res) => {
  try {
    ...
    const participants = await User.find({ _id: { $in: event.registeredUsers } });
    if (!participants)
      return res.json({
